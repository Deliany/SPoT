...
+ (NSCountedSet *)filteredTags
{
    NSArray *photos = [[self class] stanfordPhotos];
    NSCountedSet *tagsWithCount = [NSCountedSet set];
    for (NSDictionary *attr in photos) {
        NSString *stringTags = attr[FLICKR_TAGS];
        NSArray *tagsArray = [stringTags componentsSeparatedByString:@" "];
        
        [tagsWithCount addObjectsFromArray:tagsArray];
    }
    
    NSArray *excludedTags = @[@"cs193pspot",@"portrait",@"landscape"];
    for (NSString* tag in excludedTags) {
        NSUInteger count = [tagsWithCount countForObject:tag];
        while(count)
        {
            [tagsWithCount removeObject:tag];
            --count;
        }
    }
...
{
    NSArray *photos = [[self class] stanfordPhotos];
    NSMutableArray *photosForTag = [NSMutableArray array];
    
    [photos enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        NSDictionary *dic = (NSDictionary*)obj;
        NSArray *tags = [(NSString*)dic[FLICKR_TAGS] componentsSeparatedByString:@" "];
        if ([tags containsObject:tag])
        {
            [photosForTag addObject:obj];
        }
    }];
    return photosForTag;
}
